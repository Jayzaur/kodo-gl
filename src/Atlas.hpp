...
namespace kodogl
{
	...
	public:
		...
		GLuint Load( const std::string& filename, float_t size, const std::string& charset = DefaultCharset )
		{
			...
			#pragma warning ( default: 4838 )
						...
						if (!GetRegion( 5, 5, region ))
							throw AtlasException( "Atlas is full." );

						SetRegion( Region{ region.X, region.Y, 4, 4 }, data, 0 );

						AtlasGlyph glyph{ size_t( -1 ), Normalize( Region{ region.X + 2, region.Y + 2, 1, 1 } ) };
	...
	public:
		...
		/// <summary>
		/// Gets the <see cref="AtlasFont"/> with the specified name.
		/// </summary>
